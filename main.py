...
from src.monthly_rsi_gate     import MonthlyRSIGateFilter
...
class MasterOBAFPipeline:
    ...
    def __init__(self, config: dict):
        ...
        self.phase_m1     = MonthlyRSIGateFilter(config)
    ...
    def execute_daily_batch(self) -> dict:
        ...
        # ── Step 1.5: Phase -1 월봉 RSI 사전 필터 게이트 (1차 관문) ──
        logger.info("[Step 1.5] Phase -1 월봉 RSI 사전 필터 게이트 (1차 관문)")
        self.stats.set("phase_m1_in", len(target_assets))
        survivors_m1, gate_details = self.phase_m1.run_gate(target_assets)
        self.stats.set("phase_m1_out", len(survivors_m1))
        gc.collect()
        ...
        if not survivors_m1:
            logger.warning(
                f"월봉 RSI < {self.phase_m1.max_monthly_rsi} 기준을 통과한 종목이 "
                f"0개 → 이후 파이프라인(Phase 0~3) 스킵. "
                f"탈락 종목 검증은 다음 파일 참고: {gate_report_path}"
            )
            return {}

        logger.info(
            f"Phase -1 통과: {len(survivors_m1)}개 "
            f"(전체 {self.stats.get('phase_m1_in')}개 중, 월봉 RSI < {self.phase_m1.max_monthly_rsi})"
        )
        target_assets = survivors_m1
